...
<div class="container">
    <div class="d-flex">
        ...
        <a href="{{route('category.create')}}" type="button" class="btn btn-info" style="margin-left: 15px;"><i class="fas fa-plus"></i>Thêm Mới</a>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <td>
                ...
                <td>{{$cat->deleted_at ? $cat->deleted_at->format('d-m-Y') : ''}}</td>
                <td>
                    <div class="d-flex">
                    <a href="{{route('category.edit',$cat->id)}}" class="btn btn-primary" style="margin-right: 10px;"><i class="fas fa-edit"></i></a>
                    <a href="{{route('category.restore',$cat->id)}}" title="Phục hồi" class="btn btn-warning" style="margin-right: 10px;"><i class="fas fa-window-restore"></i></a>
                    <a href="{{route('category.forceDelete',$cat->id)}}" onclick="return confirm('Bạn có chắc muốn xóa không?');" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></a>
                    </div>
                </td>
            </tr>
            @endforeach
...
<form action="{{route('category.deleteAll')}}" method="POST" id="formDeleteAll" value="">
    @csrf @method('DELETE')
    <div id="idDelete"></div>
</form>
@stop()
